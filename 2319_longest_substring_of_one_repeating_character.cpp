...
private:
    ...
    Node mergeNodes(const Node& left, const Node& right)
    {
        ...
        if (left.rightChar == right.leftChar && left.prefix == left.length) res.prefix = left.length + right.prefix;
        res.suffix = right.suffix;
        if (left.rightChar == right.leftChar && right.suffix == right.length) res.suffix = right.length + left.suffix;
